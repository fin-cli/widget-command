 ...
 */
class Widget_Command extends FIN_CLI_Command {
	...
	public function list_( $args, $assoc_args ) {
		...
		if ( ! empty( $assoc_args['format'] ) && 'ids' === $assoc_args['format'] ) {
			$output_widgets = fin_list_pluck( $output_widgets, 'id' );
		}
	...
	public function add( $args, $assoc_args ) {
		...
		if ( false === $widget ) {
			FIN_CLI::error( 'Invalid widget type.' );
		}
		...
		FIN_CLI::success( 'Added widget to sidebar.' );
	}
	...
	public function update( $args, $assoc_args ) {
		...
		if ( ! $this->validate_sidebar_widget( $widget_id ) ) {
			FIN_CLI::error( "Widget doesn't exist." );
		}

		if ( empty( $assoc_args ) ) {
			FIN_CLI::error( 'No options specified to update.' );
		}
		...
		FIN_CLI::success( 'Widget updated.' );
	}
	...
	public function move( $args, $assoc_args ) {
		...
		if ( empty( $assoc_args['position'] ) && empty( $assoc_args['sidebar-id'] ) ) {
			FIN_CLI::error( 'A new position or new sidebar must be specified.' );
		}
	...
	public function deactivate( $args, $assoc_args ) {
		...
		foreach ( $args as $widget_id ) {
			if ( ! $this->validate_sidebar_widget( $widget_id ) ) {
				FIN_CLI::warning( "Widget '{$widget_id}' doesn't exist." );
				++$errors;
				continue;
			}

			list( $name, $option_index, $sidebar_id, $sidebar_index ) = $this->get_widget_data( $widget_id );
			if ( 'fin_inactive_widgets' === $sidebar_id ) {
				FIN_CLI::warning( sprintf( "'%s' is already deactivated.", $widget_id ) );
				continue;
			}
			...
			$this->move_sidebar_widget(
				$widget_id,
				$sidebar_id,
				'fin_inactive_widgets',
				$sidebar_index,
				count( $this->get_sidebar_widgets( 'fin_inactive_widgets' ) )
			);
	...
	public function delete( $args, $assoc_args ) {
		...
		foreach ( $args as $widget_id ) {
			...
			$all_widgets = $this->fin_get_sidebars_widgets();
	...
	public function reset( $args, $assoc_args ) {
		...
		if ( ! $all && empty( $args ) ) {
			FIN_CLI::error( 'Please specify one or more sidebars, or use --all.' );
		}

		// Fetch all sidebars if all flag is set.
		if ( $all ) {
			$args = array_keys( $fin_registered_sidebars );
		}

		// Sidebar ID fin_inactive_widgets is reserved by FIN core for inactive widgets.
		if ( isset( $args['fin_inactive_widgets'] ) ) {
			unset( $args['fin_inactive_widgets'] );
		}

		// Check if no registered sidebar.
		if ( empty( $args ) ) {
			FIN_CLI::error( 'No sidebar registered.' );
		}

		$count  = 0;
		$errors = 0;
		foreach ( $args as $sidebar_id ) {
			if ( ! array_key_exists( $sidebar_id, $fin_registered_sidebars ) ) {
				FIN_CLI::warning( sprintf( 'Invalid sidebar: %s', $sidebar_id ) );
				++$errors;
				continue;
			}
			...
			if ( empty( $widgets ) ) {
				FIN_CLI::warning( sprintf( "Sidebar '%s' is already empty.", $sidebar_id ) );
			} else {
				...
				FIN_CLI::log( sprintf( "Sidebar '%s' reset.", $sidebar_id ) );
				++$count;
			}
		}
	...
	private function validate_sidebar_widget( $widget_id ) {
		...
		$sidebars_widgets = $this->fin_get_sidebars_widgets();
	...
	/**
	 * Re-implementation of fin_get_sidebars_widgets()
	 * because the original has a nasty global component
	 */
	private function fin_get_sidebars_widgets() {
	 ...
	 * @return FIN_Widget|false
	 */
	private function get_widget_obj( $id_base ) {
		global $fin_widget_factory;

		$widget = fin_filter_object_list( $fin_widget_factory->widgets, array( 'id_base' => $id_base ) );
		if ( empty( $widget ) ) {
			return false;
		}
